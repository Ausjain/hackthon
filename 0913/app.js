(() => {
  ...
  if (!config?.region?.center || !config?.region?.bounds) {
    throw new Error('config.js에 유효한 지역 설정이 필요합니다.');
  }
  const { app: appConfig, region, map: mapConfig, search: searchConfig } = config;
  const REGION_CENTER = [region.center.lat, region.center.lng];
  ...
  const map = L.map('map', {
    ...
    maxBounds: REGION_BOUNDS.pad(region.maxBoundsPadding),
  ...
  map.setView(REGION_CENTER, region.initialZoom);
  ...
  function resultIsInRegion(result) {
    ...
    const matchesRegionName = region.searchAliases.some(alias => addressText.toLocaleLowerCase().includes(alias.toLocaleLowerCase()));
    return REGION_BOUNDS.contains(location) && matchesRegionName;
  }
  ...
  async function runPlaceSearch(query) {
    ...
    if (searchCache.has(normalizedQuery)) {
      ...
      renderSearchResults(cached, cached.length ? '' : `${region.name} 안에서 검색 결과를 찾지 못했어요.`);
      return;
    }
    ...
    const params = new URLSearchParams({
      q: `${query.trim()}, ${region.searchQuerySuffix}`,
      format: 'jsonv2',
      addressdetails: '1',
      namedetails: '1',
      limit: String(searchConfig.requestLimit),
      countrycodes: region.countryCode,
      'accept-language': searchConfig.language,
    ...
    try {
      ...
      const response = await fetch(`${searchConfig.endpoint}?${params}`, { signal: controller.signal });
      if (!response.ok) throw new Error(`Search failed: ${response.status}`);
      const data = await response.json();
      const results = data.filter(resultIsInRegion).slice(0, searchConfig.displayLimit);
      searchCache.set(normalizedQuery, results);
      renderSearchResults(results, results.length ? '' : `${region.name} 안에서 검색 결과를 찾지 못했어요.`);
    } catch (error) {
  ...
  function selectSearchResult(result) {
    const location = L.latLng(Number(result.lat), Number(result.lon));
    if (!resultIsInRegion(result)) {
      showToast(`${region.name} 밖의 장소는 선택할 수 없어요.`);
      return;
    }
    document.querySelector('#searchResults').hidden = true;
    map.setView(location, region.searchResultZoom);
  ...
  map.on('click', event => {
    if (!REGION_BOUNDS.contains(event.latlng)) {
      showToast(`${region.name} 안의 장소를 선택해 주세요.`);
      return;
    }
  ...
  document.querySelector('#resetViewButton').addEventListener('click', () => map.setView(REGION_CENTER, region.initialZoom));
